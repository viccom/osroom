    ...
    for file_base in uploaded_files:
        if file_base:
            ...
            if file_name:
                ...
                filename = '{}_{}.{}'.format(time_to_utcdate(time_stamp=time.time(), tformat="%Y%m%d%H%M%S"),
                                             uuid1(),
                                             file_format)
            ...
            # 文件保存的绝对路径
            save_file_path = "{}/{}/{}".format(STATIC_PATH, get_config("upload", "SAVE_DIR"), filename).replace("//", "/")
            ...
            if data != "__no_plugin__":
                ...
                key = {"key": filename, "bucket_name": None, "d": None, "type": "local"}
        else:
            key = None
...
def upload_file(file=None, prefix="", file_name=None, file_format=None,  fetch_url=False,
    ...
    if file_name:
        ...
        filename = '{}-{}.{}'.format(time_to_utcdate(time_stamp=time.time()), uuid1(), file_format)
    ...
    if fetch_url:
        ...
        if data != "__no_plugin__":
            ...
            result = {"key":filename, "bucket_name":None, "d":None, "type":"local"}
...
def copy_file(from_file_url_obj=None, from_path=None, replica_prefix="", replica_file_name=None, replica_file_format=None):
    ...
    if replica_file_name:
        ...
        filename = '{}-{}.{}'.format(time_to_utcdate(time_stamp=time.time()), uuid1(), replica_file_format)
...
def allowed_file(filename):
    ...
    return os.path.splitext(filename)[1].strip(".").lower() in get_config("upload", "UP_ALLOWED_EXTENSIONS")