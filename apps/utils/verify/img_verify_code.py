    ...
    def create_point(self, num):
        ...
        for i in range(num):
            self.draw.point(
                ...
                fill=self.random_color.random_color()
            )
...
class RandomColor():
    ...
    def random_color2(self):
        ...
        :return:
        '''
        return (random.randint(32, 180), random.randint(64, 180), random.randint(64, 180))
...
def create_img_code(interference=0):
    ...
    _code = {'url':code_url_info, 'str':_str,'time':time.time(), "type":"image"}
...
def verify_image_code(code_img_obj, code):
    ...
    '''
    r = False
    if not "key" in code_img_obj:
        return r

    expiration_time = get_config("verify_code", "EXPIRATION")
    code_data = mdb_web.db.verify_code.find_one({'url.key':code_img_obj["key"], "type":"image"})
    if code_data:
        if code.lower() == code_data['str'].lower() and time.time()-code_data['time'] < expiration_time:
            r = True
    return r
...
def vercode_del(file_url_obj=None, expiration_time=None):
    ...
    if file_url_obj:
        ...
        mdb_web.db.verify_code.delete_many({'url.key':file_url_obj["key"], "type":"image"})

    if expiration_time:
        codes = mdb_web.db.verify_code.find({'time': {"$lt":time.time()-expiration_time},
                                             "type": "image"})
        for code in codes:
            file_del(code["url"])
