...
def update_plugin_setting():
    ...
    s, r = arg_verify(reqargs=[("plugin name", plugin_name), ("key", key)], required=True)
    if not s:
        return r


    old_conf = mdb_sys.db.plugin_config.find_one({"key": key, "plugin_name": plugin_name})
    if not old_conf:
        ...
        try:
            if old_conf["value_type"] == "int" or old_conf["value_type"] == "binary":
                ...
                if not isinstance(value, dict):
                    value = json.loads(value)
                if not isinstance(value, dict):
                    data = {"msg":gettext('The format of the "value" errors, need a "{}" type').format(old_conf["value_type"]),
                ...
                data = {"msg":gettext('There is no {}').format(old_conf["value_type"]),
                        "msg_type":"e", "http_status":400}
                return data
...
def refresh_plugin_setting():
    ...
    if register_r:
        if mdb_sys.db.plugin.find_one({"plugin_name": plugin_name, "active": {"$nin": [1, True]}}):
            # 刷新后, 判断如果并位激活的插件,那就注销注册
            plugin_manager.unregister_plugin(plugin_name)
        data = {"msg": gettext("Refreshed successfully"), "msg_type": "s", "http_status": 201}
    else:
        data = {"msg": gettext("Failed to refresh. Please check the previous plug-in page for error messages"),
                "msg_type": "e", "http_status": 400}
...
def install_require_package():
    ...
    if not s:
        return r
    plugin_req_file_path = "{}/requirements.txt".format(os.path.join(PLUG_IN_FOLDER, plugin_name))
    ...
    for host in hosts:
        ...
        if not s:
            connection_failed.append({"host_ip": host_info["local_ip"], "error": r})
            continue
        else:
            try:
                ...
                connection_failed.append({"host_ip": host_info["local_ip"], "error":str(e) })
                continue
            if not ssh:
                connection_failed.append({"host_ip":host_info["local_ip"], "error":"Failed to connect to server host"})
                continue
        ssh.close()
    ...
    if connection_failed:
        # 更新插件需求包安装状态
        plugin = mdb_sys.db.plugin.find_one({"plugin_name": plugin_name}, {"require_package_install_result": 1})
        ...
        mdb_sys.db.plugin.update_one({"plugin_name": plugin_name},
                                     {"$set": {"require_package_install_result": plugin["require_package_install_result"]}
                                      })

        data = {"msg":gettext("Some host connections failed. Successfully connected host has installed requirements package in the background"),
                "data":connection_failed,
                "msg_type":"w", "http_status":201}
    else:
        data = {"msg":gettext("Executed related installation commands in the background"),
...
def install_process(plugin_name, host_info, packages):
    ...
    if ssh:
        result = []
        venv = ". {}/bin/activate && ".format(get_config("py_venv", "VENV_PATH"))
