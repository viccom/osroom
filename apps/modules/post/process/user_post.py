...
def post_issue():
    ...
    try:
        ...
        category = None
    # Title 处理
    s, r = arg_verify(reqargs=[(gettext("title"), title.strip())], max_len=get_config("post", "TITLE_MAX_LEN"),
                      required=True)
    if not s:
        return r
    # content
    s, r = arg_verify(reqargs=[(gettext("content"), content.strip()), ("editor", editor)], required=True)
    if not s:
        return r
    ...
    s, r = arg_verify(reqargs=[(gettext("content"), content_text)], max_len=int(get_config("post", "MAX_LEN")))
    if not s:
        data = r
    else:
        if issue_way == "issue":
        ...
        if id:
            # 文章更新
            post = mdb_web.db.post.find_one({"_id":ObjectId(id), "user_id":current_user.str_id})
            if post["issue_time"]:
                # 有发布时间，则发布时间不改变
        ...
        if id:
            mdb_web.db.post.update_one({"_id":ObjectId(id), "user_id":current_user.str_id}, {"$set":post}, upsert=True)
        else:
            post["comment_num"] = 0
            post["like"] = 0
        ...
        if audited and issue_way and audit_score >= get_config("content_inspection", "ALLEGED_ILLEGAL_SCORE"):
            insert_user_msg(user_id=post["user_id"], ctype="notice", label="audit_failure",
                            title=gettext("[Label:{}]Post allegedly violated").format(audit_label), content={"text": post["brief_content"]},
                            target_id=str(id), target_type="post")
        if issue_way:
            data = {"msg":gettext("Issue success"), "msg_type":"s", "http_status":201}
        else:
...
def post_restore():
    ...
    if r.modified_count:
        ...
        data = {"msg":gettext("Restore failed"), "msg_type":"w", "http_status":400}
    return data