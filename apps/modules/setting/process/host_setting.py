...
def sys_host_edit():
    ...
    msg = None
    s, r = arg_verify(reqargs=[("IP", ip),(gettext("username"),username),(gettext("password"),password)],
                      required=True)
    if not s:
        return r
    if msg:
        ...
        if r.modified_count:
            data ={"msg":gettext("The update is successful"), "msg_type":"s", "http_status":201}
        else:
...
def sys_host_delete():
    ...
    if r.deleted_count:
        data ={"msg":gettext("Successfully deleted {} host information").format(r.deleted_count),
...
def sys_host_exec_cmd():
    ...
    if host:
        ...
        for cmd in exec_cmd.split("\n"):
            ...
            if re.search("^sudo\s*.*", cmd) or re.search(".*\s+sudo\s+.*", cmd):
                result.append([gettext('[Warning]: "sudo" command go to the server')])
            elif re.search("^rm\s+.*", cmd) or re.search(".*\s+rm\s+.*", cmd):
                result.append([gettext('[Warning]: "rm" command go to the server')])
            else:
    ...
    if host:
        ...
        if not s:
            ...
            try:
                ...
                return data
            if not ssh:
                data = {"msg": gettext("Connection host[{}] failed").format(ip),
                        "msg_type": "e", "http_status":400}
            else:

                data = {"msg":gettext("Successfully connecting server host"), "msg_type":"s", "http_status":201}
    else:
        data = {"msg": gettext("Host does not exist"), "msg_type": "e", "http_status": 400}
    return data