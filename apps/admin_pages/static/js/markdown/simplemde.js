...
function drawImage(editor) {
	...
	//图片上传 by osroom
	//触发文件选择
	window.Up_g_editor = editor;
	var fileElem = document.getElementById("upfile-img");
	if (fileElem) {
        fileElem.click();
    }
}

function insertImgUrl(urls) {
    // 插入图片链接到ｍｄ中
    var cm = window.Up_g_editor.codemirror;
    var stat = getState(cm);
    var options = window.Up_g_editor.options;
    ...
    if(options.promptURLs) {
        ...
        var url = "\n[comment]: <> (注意:1. html标签和md标记间需要空一行. 2.需要调整图片大小可以改成:<img src='' width='80%' height='auto'>)\n"
        for(var j = 0,len=urls.length; j < len; j++) {

              //url = url +"; <img src='"+urls[j]+"' width='80%' height='auto' style='text-align: center;'>\n"+ urls[j].split("/").pop()+"*\n";
              url = url + '<div align="center">\n\n'
                    +"![]("
                    + urls[j] + ')\n\n</div>\n<div align="center">\n\n*'
                    + urls[j].split("/").pop()
                    +"*\n\n</div>\n";
...
function SimpleMDE(options) {
	...
	this.insertImgUrl = insertImgUrl;
}
...
SimpleMDE.insertImgUrl = insertImgUrl;
