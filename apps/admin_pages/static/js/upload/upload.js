...
function init_upload(webupload_conf, before_send_header) {
                    ...
                    window['expressinstallcallback'] = function( state ) {
                        switch(state) {
                            case 'Download.Cancelled':
                                alert_msg({msg:$("#js-tr-cancle-update").attr("content"), msg_type:"w"});
                                break;

                            case 'Download.Failed':
                                alert_msg({msg:$("#js-tr-install-failed").attr("content"), msg_type:"w"});
                                break;

                            default:
                                alert_msg({msg:$("#js-tr-install-success").attr("content"), msg_type:"s"});
                                break;
                        }
        ...
        var upload_conf = {
            pick: {
                id: '#filePicker',
                label: $("#js-tr-up-select-file").attr("content")
            },
            formData:{
                name:"No_name",
                category:"test",
...
//        });
        ...
        function addFile( file ) {
            var $li = $( '<li id="' + file.id + '">' +
                ...
                $btns = $('<div class="file-panel">' +
                    '<span class="cancel">'+$("#js-tr-delete").attr("content")+'</span>' +
                    '<span class="rotateRight">'+$("#js-tr-rotate-left").attr("content")+'</span>' +
                    '<span class="rotateLeft">'+$("#js-tr-rotate-left").attr("content")+'</span></div>').appendTo( $li ),
            ...
            if ( file.getStatus() === 'invalid' ) {
                ...
                // @todo lazyload
                $wrap.text( $("#js-tr-in-preview").attr("content") );
                uploader.makeThumb( file, function( error, src ) {
                    var img;

                    if ( error ) {
                        $wrap.text( $("#js-tr-in-can-t-preview").attr("content")  );
                        return;
                    }
        ...
        function updateStatus() {
            ...
            if ( state === 'ready' ) {
                text = $("#js-tr-files-num").attr("content")+': ' + fileCount + ','+$("#js-tr-total").attr("content") +
                ...
                if ( stats.uploadFailNum ) {
                    ...
                    stats.uploadFailNum+' '
                    + $("#js-tr-upfailed-num").attr("content")+' '
                    +'<a class="retry" href="#">'
                    +$("#js-tr-pause-re-upload").attr("content")
                    +'</a>'
                    + ' '+$("#js-tr-or").attr("content")+' '
                    +'<a class="ignore" href="#">'
                    +$("#js-tr-ignore").attr("content")
                    +'</a>'
                }
        ...
        function setState( val ) {
            ...
            switch ( state ) {
                ...
                case 'uploading':
                    ...
                    $progress.show();
                    $upload.text( $("#js-tr-pause-uploading").attr("content") );
                    break;

                case 'paused':
                    $progress.show();
                    $upload.text( $("#js-tr-continue-upload").attr("content") );
                    break;

                case 'confirm':
                    $progress.hide();
                    $( '#filePicker2' ).removeClass( 'element-invisible' );
                    $upload.text( $("#js-tr-start-upload").attr("content"));
